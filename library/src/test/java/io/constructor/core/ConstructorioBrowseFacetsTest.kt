...
class ConstructorioBrowseFacetsTest {
    ...
    fun getBrowseFacetsResults() {
        ...
        val path =
                "/browse/facets?key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.28.0"
    ...
    fun getBrowseFacetsResultsWithOptionalParameters() {
        ...
        val request = mockServer.takeRequest()
        val path =
                "/browse/facets?page=5&offset=10&num_results_per_page=20&fmt_options%5Bshow_hidden_facets%5D=true&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.28.0"
        assert(request.path!!.startsWith(path))
    }
