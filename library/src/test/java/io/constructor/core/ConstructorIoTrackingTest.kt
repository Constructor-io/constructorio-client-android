...
class ConstructorIoTrackingTest {
    ...
    fun trackSessionStart() {
        ...
        val path = "/behavior?action=session_start&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
    ...
    fun trackInputFocus() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/behavior?term=tita&action=focus&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun trackAutocompleteSelect() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/select?section=Search%20Suggestions&original_query=tit&tr=click&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun trackAutocompleteSelectWithSectionAndResultID() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/select?section=Search%20Suggestions&original_query=tit&tr=click&group%5Bgroup_id%5D=123123&group%5Bdisplay_name%5D=recommended&result_id=2346784&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun trackSearchSubmit() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/search?original_query=tit&tr=search&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun trackSearchResultLoaded() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/behavior?term=titanic&num_results=10&action=search-results&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun trackSearchResultLoadedWithCustomerIDs() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/behavior?term=titanic&num_results=10&customer_ids=TIT-REP-1997%2CQE2-REP-1969&action=search-results&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun trackSearchResultClick() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/click_through?name=titanic%20replica&customer_id=TIT-REP-1997&section=Products&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun trackSearchResultClickWithSectionAndResultID() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/click_through?name=titanic%20replica&customer_id=TIT-REP-1997&section=Products&result_id=3467632&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun trackConversion() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/v2/behavioral_action/conversion?key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
    ...
    fun trackPurchase() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/purchase?section=Products&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
    ...
    fun trackPurchaseWithSection() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/purchase?section=Recommendations&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assertEquals("[{item_id:TIT-REP-1997},{item_id:QE2-REP-1969}]", requestBody["items"])
        assertEquals("ORD-1312343", requestBody["order_id"])
        assertEquals("12.99", requestBody["revenue"])
        assertEquals("POST", request.method)
    ...
    fun trackBrowseResultLoaded() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/browse_result_load?key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
    ...
    fun trackBrowseResultClick() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/browse_result_click?section=Products&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assertEquals("group_id", requestBody["filter_name"])
        assertEquals("Movies", requestBody["filter_value"])
        assertEquals("TIT-REP-1997", requestBody["item_id"])
    ...
    fun trackBrowseResultClickWithSectionAndResultID() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/browse_result_click?section=Products&result_id=3467632&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assertEquals("group_id", requestBody["filter_name"])
        assertEquals("Movies", requestBody["filter_value"])
        assertEquals("TIT-REP-1997", requestBody["item_id"])
        assertEquals("4", requestBody["result_position_on_page"])
    ...
    fun trackRecommendationResultClick() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/recommendation_result_click?section=Products&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
    ...
    fun trackRecommendationResultClickWithSectionAndResultID() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/recommendation_result_click?section=Search%20Suggestions&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assertEquals("pdp5", requestBody["pod_id"])
        assertEquals("User Featured", requestBody["strategy_id"])
        assertEquals("TIT-REP-1997", requestBody["item_id"])
        assertEquals("POST", request.method)
    ...
    fun trackRecommendationResultsView() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/recommendation_result_view?section=Products&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assertEquals("pdp5", requestBody["pod_id"])
    ...
    fun trackRecommendationResultsViewWithSectionAndResultID() {
        ...
        val request = mockServer.takeRequest()
        val requestBody = getRequestBody(request)
        val path = "/v2/behavioral_action/recommendation_result_view?section=Search%20Suggestions&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.18.4&_dt="
        assertEquals("pdp5", requestBody["pod_id"])
        assertEquals("4", requestBody["num_results_viewed"])
