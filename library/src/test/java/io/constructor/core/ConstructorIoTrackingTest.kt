...
class ConstructorIoTest {
    ...
    fun trackSessionStart() {
        ...
        val path = "/behavior?action=session_start&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
    ...
    fun trackInputFocus() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/behavior?term=tita&action=focus&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
        assert(request.path.startsWith(path))
    }
    ...
    fun trackAutocompleteSelect() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/select?autocomplete_section=Search%20Suggestions&original_query=tit&tr=click&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt="
        assert(request.path.startsWith(path))
    }
    ...
    fun trackAutocompleteSelectWithSectionAndResultID() {
        val mockResponse = MockResponse().setResponseCode(204)
        mockServer.enqueue(mockResponse)
        val observer = ConstructorIo.trackAutocompleteSelectInternal("titanic", "tit", "Search Suggestions", Group("recommended", "123123"), "2346784").test()
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/select?autocomplete_section=Search%20Suggestions&original_query=tit&tr=click&group%5Bgroup_id%5D=123123&group%5Bdisplay_name%5D=recommended&result_id=2346784&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt="
        assert(request.path.startsWith(path))
    }
    ...
    fun trackSearchSubmit() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/search?original_query=tit&tr=search&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
        assert(request.path.startsWith(path))
    }
    ...
    fun trackSearchResultLoaded() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/behavior?term=titanic&num_results=10&action=search-results&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
        assert(request.path.startsWith(path))
    }
    ...
    fun trackSearchResultClick() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/click_through?name=titanic%20replica&customer_id=TIT-REP-1997&autocomplete_section=Products&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
        assert(request.path.startsWith(path))
    }
    ...
    fun trackSearchResultClickWithSectionAndResultID() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/titanic/click_through?name=titanic%20replica&customer_id=TIT-REP-1997&autocomplete_section=Products&result_id=3467632&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
        assert(request.path.startsWith(path))
    }
    ...
    fun trackConversion() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/TERM_UNKNOWN/conversion?name=titanic%20replica&customer_id=TIT-REP-1997&revenue=89.00&autocomplete_section=Products&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
        assert(request.path.startsWith(path))
    }
    ...
    fun trackPurchase() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/TERM_UNKNOWN/purchase?customer_ids=TIT-REP-1997&customer_ids=QE2-REP-1969&revenue=12.99&order_id=ORD-1312343&autocomplete_section=Products&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
        assert(request.path.startsWith(path))
    }
    ...
    fun trackPurchaseWithSection() {
        ...
        observer.assertComplete()
        val request = mockServer.takeRequest()
        val path = "/autocomplete/TERM_UNKNOWN/purchase?customer_ids=TIT-REP-1997&customer_ids=QE2-REP-1969&revenue=12.99&order_id=ORD-1312343&autocomplete_section=Recommendations&key=copper-key&i=wacko-the-guid&ui=player-three&s=67&c=cioand-2.1.0&_dt=";
        assert(request.path.startsWith(path))
    }
}
