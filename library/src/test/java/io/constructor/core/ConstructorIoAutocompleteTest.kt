...
class ConstructorIoAutocompleteTest {
    ...
    fun getAutocompleteResults() {
        ...
        val path =
            "/autocomplete/titanic?key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.45.0&_dt="
    ...
    fun getAutocompleteResultsWithFacetFilter() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/autocomplete/titanic?filters%5BstoreLocation%5D=CA&key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.45.0&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getAutocompleteResultsWithSectionFacetFilters() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/autocomplete/titanic?filters%5BstoreLocation%5D=CA&filters%5BSearch%20Suggestions%5D%5BstoreLocation%5D=US&filters%5BProducts%5D%5Bbrand%5D=Top%20Brand&key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.45.0&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getAutocompleteResultsWithGroupIdFilter() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/autocomplete/titanic?filters%5Bgroup_id%5D=101&key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.45.0&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getAutocompleteResultsWithHiddenFields() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/autocomplete/bbq?fmt_options%5Bhidden_fields%5D=hiddenField1&fmt_options%5Bhidden_fields%5D=hiddenField2&key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.45.0&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getAutocompleteResultsWithProperUrlEncoding() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/autocomplete/2%25%20cheese?key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.45.0&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getAutocompleteResultsWithFiltersUsingBuilder() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/autocomplete/titanic?filters%5BstoreLocation%5D=CA&filters%5Bgroup_id%5D=101&key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.45.0&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getAutocompleteResultsWithNumResultsPerSectionUsingBuilder() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/autocomplete/titanic?num_results_Products=5&num_results_Search%20Suggestions=10&key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.45.0&_dt="
        assert(request.path!!.startsWith(path))
    }
