...
class ConstructorIoBrowseTest {
    ...
    fun getBrowseResults() {
        ...
        val path = "/browse/group_id/Beverages?key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.6.0&_dt"
    ...
    fun getBrowseResultsWithHiddenFields() {
        val mockResponse = MockResponse().setResponseCode(200).setBody(TestDataLoader.loadAsString("browse_response.json"))
        mockServer.enqueue(mockResponse)
        constructorIo.getBrowseResults("group_id", "Beverages", null, null, null, null, null, null, arrayOf("hiddenField1", "hiddenField2")).test()
        val request = mockServer.takeRequest()
        val path = "/browse/group_id/Beverages?hidden_fields=hiddenField1&hidden_fields=hiddenField2&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.6.0&_dt"
        assert(request.path.startsWith(path))
    }
}