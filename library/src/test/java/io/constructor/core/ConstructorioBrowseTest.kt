...
class ConstructorIoBrowseTest {
    ...
    fun getBrowseResults() {
        ...
        val path =
            "/browse/group_id/Beverages?key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.18.6&_dt"
    ...
    fun getBrowseResultsWithSection() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/browse/group_id/Beverages?section=Sold%20Out&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.18.6&_dt"
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getBrowseResultsWithFacets() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/browse/group_id/Beverages?filters%5BBrand%5D=Signature%20Farms&filters%5BBrand%5D=Del%20Monte&filters%5BNutrition%5D=Organic&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.18.6&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getBrowseResultsWithHiddenFields() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/browse/group_id/Beverages?fmt_options%5Bhidden_fields%5D=hiddenField1&fmt_options%5Bhidden_fields%5D=hiddenField2&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.18.6&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getBrowseResultsWithHiddenFacets() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/browse/group_id/Beverages?fmt_options%5Bhidden_facets%5D=Brand&fmt_options%5Bhidden_facets%5D=price_US&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.18.6&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getBrowseResultsWithCollection() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/browse/collection_id/test-collection?key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.18.6&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getBrowseResultWithGroupsSort() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/browse/group_id/Beverages?fmt_options%5Bgroups_sort_by%5D=value&fmt_options%5Bgroups_sort_order%5D=ascending&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.18.6&_dt="
        assert(request.path!!.startsWith(path))
    }
