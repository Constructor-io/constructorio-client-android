...
class ConstructorIoSearchTest {
    ...
    fun getSearchResults() {
        ...
        val path =
            "/search/corn?key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
    ...
    fun getSearchResultsWithRedirect() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/bbq?key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithSection() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/bbq?section=Sold%20Out&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithFacets() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/bbq?filters%5BBrand%5D=Signature%20Farms&filters%5BBrand%5D=Del%20Monte&filters%5BNutrition%5D=Organic&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1"
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithHiddenFields() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/bbq?fmt_options%5Bhidden_fields%5D=hiddenField1&fmt_options%5Bhidden_fields%5D=hiddenField2&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithHiddenFacets() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/bbq?fmt_options%5Bhidden_facets%5D=Brand&fmt_options%5Bhidden_facets%5D=price_US&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithProperUrlEncoding() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/2%25%20cheese?key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithGroupsSort() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/bbq?fmt_options%5Bgroups_sort_by%5D=value&fmt_options%5Bgroups_sort_order%5D=descending&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithPreFilterExpression() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/bbq?fmt_options%5Bgroups_sort_by%5D=value&fmt_options%5Bgroups_sort_order%5D=descending&pre_filter_expression=%7B%22and%22%3A%5B%7B%22name%22%3A%22Country%22%2C%22value%22%3A%22US%22%7D%5D%7D&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithGroupsSortUsingBuilder() {
        ...
        val request = mockServer.takeRequest()
        val path =
            "/search/bbq?fmt_options%5Bgroups_sort_by%5D=value&fmt_options%5Bgroups_sort_order%5D=ascending&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.31.1&_dt="
        assert(request.path!!.startsWith(path))
    }
