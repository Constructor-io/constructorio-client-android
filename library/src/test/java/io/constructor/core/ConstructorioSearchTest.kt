...
class ConstructorIoSearchTest {
    ...
    fun getSearchResultsWithHiddenFields() {
        ...
        val request = mockServer.takeRequest()
        val path = "/search/bbq?fmt_options%5Bhidden_fields%5D=hiddenField1&fmt_options%5Bhidden_fields%5D=hiddenField2&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.13.0&_dt="
        assert(request.path!!.startsWith(path))
    }
    ...
    fun getSearchResultsWithHiddenFacets() {
        ...
        val request = mockServer.takeRequest()
        val path = "/search/bbq?fmt_options%5Bhidden_facets%5D=Brand&fmt_options%5Bhidden_facets%5D=price_US&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.13.0&_dt="
        assert(request.path!!.startsWith(path))
    }
}