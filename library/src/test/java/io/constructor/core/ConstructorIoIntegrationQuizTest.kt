...
class ConstructorIoIntegrationQuizTest {
    ...
    fun getQuizNextQuestionCRTAgainstRealResponse() {
        runBlocking {
            ...
            val quizResult = constructorIo.getQuizNextQuestionCRT("test-quiz", answers, quizSessionId = "bc48a85d-2f45-4c91-ba3a-dcf655b33831")
            assertNotNull(quizResult?.quizId)
            assertNotNull(quizResult?.quizVersionId)
            assertNotNull(quizResult?.quizSessionId)
            assertEquals(quizResult?.nextQuestion?.id, 4)
    ...
    fun getQuizResultsCRTAgainstRealResponse() {
        runBlocking {
            ...
            val quizResult = constructorIo.getQuizResultsCRT("test-quiz", answers, quizSessionId = "bc48a85d-2f45-4c91-ba3a-dcf655b33831")
            assertEquals(quizResult?.quizId, "test-quiz");
            assertNotNull(quizResult?.quizVersionId)
            assertNotNull(quizResult?.quizSessionId)
            assertNotNull(quizResult?.request)
    ...
    fun trackQuizResultClickAgainstRealResponse() {
        val observer = ConstructorIo.trackQuizResultClickInternal("test-quiz", "23AECMA-1EFKCI", "34NCUIEI-214CDN", "shirt-a", "shirt-a--reg", "White shirt", "Products", "123", 10, 10, 1, 10).test();
        observer.assertComplete()
        Thread.sleep(timeBetweenTests)
    }
    ...
    fun trackQuizResultLoadAgainstRealResponse() {
        val observer = ConstructorIo.trackQuizResultLoadInternal("test-quiz", "23AECMA-1EFKCI", "34NCUIEI-214CDN", "Products", "123", 1, 10).test();
        observer.assertComplete()
        Thread.sleep(timeBetweenTests)
    }
    ...
    fun trackQuizConversionAgainstRealResponse() {
        val observer = ConstructorIo.trackQuizConversionInternal("coffee-quiz", "23AECMA-1EFKCI", "34NCUIEI-214CDN", null, null, null, "shirt-a", "shirt-a--reg", "White shirt", "Products", "129.99").test();
        observer.assertComplete()
        Thread.sleep(timeBetweenTests)
    }
}
