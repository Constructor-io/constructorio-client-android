...
class ConstructorIoRecommendationsTest {
    ...
    fun getRecommendationResults() {
        ...
        val path = "/recommendations/v1/pods/titanic?key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.26.1&_dt="
    ...
    fun getRecommendationResultsWithMultipleItemIdsUsingBuilder() {
        ...
        val request = mockServer.takeRequest()
        val path = "/recommendations/v1/pods/titanic?item_id=item_id_1&item_id=item_id_2&key=golden-key&i=guido-the-guid&ui=player-one&s=79&c=cioand-2.26.1&_dt="
        assert(request.path!!.startsWith(path))
    }
