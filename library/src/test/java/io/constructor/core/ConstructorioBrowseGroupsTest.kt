...
class ConstructorioBrowseGroupsTest {
    ...
    fun getBrowseGroupsResults() {
        ...
        val path =
                "/browse/groups?key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.25.2"
    ...
    fun getBrowseGroupsResultsWithOptionalParameters() {
        ...
        val request = mockServer.takeRequest()
        val path =
               "/browse/groups?filters%5Bgroup_id%5D=Brand&fmt_options%5Bgroups_max_depth%5D=1&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.25.2"
        assert(request.path!!.startsWith(path))
    }
