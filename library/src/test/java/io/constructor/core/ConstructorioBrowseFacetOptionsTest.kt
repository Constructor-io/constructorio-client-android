...
class ConstructorioBrowseFacetOptionsTest {
    ...
    fun getBrowseFacetOptionsResults() {
        ...
        val path =
                "/browse/facet_options?facet_name=brands&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.46.0"
    ...
    fun getBrowseFacetOptionsResultsWithOptionalParameters() {
        ...
        val request = mockServer.takeRequest()
        val path =
                "/browse/facet_options?fmt_options%5Bshow_hidden_facets%5D=true&facet_name=Brands&key=silver-key&i=guapo-the-guid&ui=player-two&s=92&c=cioand-2.46.0"
        assert(request.path!!.startsWith(path))
    }
