...
object Mapper {
    ...
    fun toSuggestionsViewModel(suggestions: List<Suggestion>): List<SuggestionViewModel> {
        val data = ArrayList<SuggestionViewModel>()
        suggestions.mapIndexed { index, suggestion ->
            if (index == 0) {
                suggestion.data.groups.mapTo(data) { SuggestionViewModel(suggestion.value, it, Constants.QueryValues.SEARCH_SUGGESTIONS, suggestion.matchedTerms) }
            } else {
